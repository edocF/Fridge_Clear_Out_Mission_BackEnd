...
public class RecipeController {
    ...
    @GetMapping("/foods")
    public Result getUserFoods() {
        try {
            List<Food> foods = foodService.getFoodList();
            // 转换为前端需要的格式
            List<FoodInfoForRecipe> foodInfoForRecipes = foods.stream()
                .map(food -> new FoodInfoForRecipe(
                    food.getId(),
                    food.getName(),
                    food.getExpired(),
                    food.getExpSoon(),
            ...
            return Result.success(foodInfoForRecipes);
        } catch (Exception e) {
            log.error("获取用户食材失败", e);
            return Result.error("获取食材失败: " + e.getMessage());
        }
    }
}
